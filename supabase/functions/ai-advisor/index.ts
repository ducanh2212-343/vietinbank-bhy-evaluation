...
// Trần TỔNG toàn hệ thống/giờ — chốt chặn chống cháy credit do vòng lặp/lạm dụng,
// KHÔNG nhằm bóp nhu cầu bình thường. Override qua env AI_GLOBAL_RATE_LIMIT_PER_HOUR.
const RATE_LIMIT_GLOBAL_PER_HOUR = Number(Deno.env.get('AI_GLOBAL_RATE_LIMIT_PER_HOUR')) || 1000;
...
// Timeout cho mọi lệnh gọi tới nhà cung cấp AI. Nếu provider treo/không phản hồi
// thì hủy để không giữ slot edge function tới hết wall-clock. Override qua env.
const AI_FETCH_TIMEOUT_MS = Number(Deno.env.get('AI_FETCH_TIMEOUT_MS')) || 60_000;

/**
 * fetch có timeout bằng AbortController. Bộ đếm được clear ngay khi nhận được
 * response headers, nên với streaming (SSE) phần thân vẫn stream bình thường —
 * timeout chỉ áp cho giai đoạn kết nối/first-byte, đúng thứ cần chặn khi treo.
 */
async function fetchWithTimeout(
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      throw new Error(
        `Nhà cung cấp AI không phản hồi trong ${Math.round(timeoutMs / 1000)}s. Vui lòng thử lại sau.`,
      );
    }
    throw e;
  } finally {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'test_connection') {
      ...
      const testRes = await fetchWithTimeout(providerCfg.endpoint, {
        method: 'POST',
    ...
    try {
      ...
      // Trần tổng toàn hệ thống — chặn cháy credit khi nhiều user cùng dồn dập.
      if (!rlErr) {
        const { count: globalCount, error: gErr } = await adminCli
          .from('ai_usage_log')
          .select('id', { count: 'exact', head: true })
          .gte('created_at', oneHourAgo);
        if (!gErr && (globalCount ?? 0) >= RATE_LIMIT_GLOBAL_PER_HOUR) {
          return new Response(JSON.stringify({ error: 'Hệ thống đã đạt giới hạn lượt AI toàn cơ quan trong 1 giờ. Vui lòng thử lại sau ít phút.' }), {
            status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      }
...
Hãy gọi tool select_courses để trả về tất cả khóa phù hợp, sắp xếp theo relevance_score giảm dần.`;
      ...
      const aiRes2 = await fetchWithTimeout(providerCfg.endpoint, {
        method: 'POST',
        headers: { Authorization: `Bearer ${providerCfg.apiKey}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model,
...
Gọi tool propose_criteria để trả kết quả.`;
      ...
      const critRes = await fetchWithTimeout(providerCfg.endpoint, {
        method: 'POST',
        headers: { Authorization: `Bearer ${providerCfg.apiKey}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model,
