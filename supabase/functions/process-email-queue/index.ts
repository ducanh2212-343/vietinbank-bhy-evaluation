...
// Gửi qua Resend (dùng khi RESEND_API_KEY được cấu hình → thoát dịch vụ email của Lovable).
// Ném lỗi KÈM mã trạng thái để logic retry/rate-limit hiện có (nhận diện 429/403 trong message) hoạt động đúng.
async function sendViaResend(resendKey: string, payload: Record<string, any>) {
...
Deno.serve(async (req) => {
  ...
  // Cần ÍT NHẤT một nhà cung cấp gửi email (Resend ưu tiên; nếu không có thì fallback Lovable).
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        // Ưu tiên Resend (email của riêng bạn) nếu đã cấu hình RESEND_API_KEY; nếu chưa, dùng Lovable.
        if (resendKey) {
