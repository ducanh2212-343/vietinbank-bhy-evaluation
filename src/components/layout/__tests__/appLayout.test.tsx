...
describe('Khung ứng dụng', () => {
  beforeEach(() => {
    ...
    mockAuth.guestScreens = ['trang-chu', 'tin-tuc', 'sharing', 'connect'];
  ...
  it('chạm tab Chiêu thức 3 bung CÂY THU GỌN, không đổ phẳng ~49 mục', async () => {
    ...
    expect(within(tam).queryByText('Quản trị AI & Prompt')).not.toBeInTheDocument();
    // Nhưng bung thư mục ra là thấy
    fireEvent.click(within(tam).getByRole('button', { name: /Bản tin & Trợ lý AI/ }));
    expect(within(tam).getByText('Quản trị AI & Prompt')).toBeInTheDocument();
  });
  ...
  it('khách đối tác KHÔNG thấy Cây Ký Ức khi chưa được mở màn hình này', () => {
    mockAuth.isGuest = true;
    dungKhung('/one');
    const thanhNav = screen.getByLabelText('Điều hướng chính cổng BHY ONE');
  ...
  it('khách được Phòng TCTH mở Cây Ký Ức thì thấy tab ngay trên thanh', () => {
    // Quyền xem của khách nay do màn «Quản trị tài khoản khách» quyết định —
    // mở thêm một màn là menu tự có, không phải phát hành bản mới
    mockAuth.isGuest = true;
    mockAuth.guestScreens = ['trang-chu', 'cay-ky-uc'];
    dungKhung('/one');
    const thanhNav = screen.getByLabelText('Điều hướng chính cổng BHY ONE');
    expect(within(thanhNav).getByRole('link', { name: 'Cây Ký Ức' })).toBeInTheDocument();
  });
  ...
  it('khách đối tác: không có menu dọc và không thấy phân hệ 343', () => {
    ...
    expect(within(thanhNav).queryByRole('button', { name: 'Quản trị chung' })).not.toBeInTheDocument();
  });

  it('quản trị viên thấy khu Quản trị chung trên thanh chính', () => {
    mockAuth.isAdmin = true;
    dungKhung('/tong-quan');
    const thanhNav = screen.getByLabelText('Điều hướng chính cổng BHY ONE');
    expect(within(thanhNav).getByRole('button', { name: 'Quản trị chung' })).toBeInTheDocument();
  });

  it('cán bộ thường KHÔNG thấy khu Quản trị chung', () => {
    dungKhung('/tong-quan');
    const thanhNav = screen.getByLabelText('Điều hướng chính cổng BHY ONE');
    expect(within(thanhNav).queryByRole('button', { name: 'Quản trị chung' })).not.toBeInTheDocument();
  });
