...
/**
 * Engine làm bài kiểu Duolingo dùng chung cho quiz phòng và chiến dịch chi
...
export function QuizPlayEngine({ start, answer, backPath, resultsPath, completionNote }: QuizPlayEngineProps) {
  ...
  const submitAnswer = async (index: number | null) => {
    ...
    clearTimer();
    setSelected(index);
    const { data, error } = await answer(attemptId, index);
    setSubmitting(false);
    if (error) {
          ...
          <CardContent className="py-8 flex flex-col items-center gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {completionNote || 'Bạn đã giữ nhịp học tuần này — chuỗi vẫn cháy 🔥'}
              </p>
            </div>
