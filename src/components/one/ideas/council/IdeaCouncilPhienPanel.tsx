...
import { ListPlus, Play, Square, Trash2, Users } from 'lucide-react';
import {
...
import { useCouncilMembers, useCouncilMutations, useDiemDanh, type CouncilItem, type CouncilRound } from './useIdeaCouncil';
...
//
// ĐIỂM DANH THEO PHIÊN (16/09/2026): người đến muộn / về sớm được đánh dấu vắng
// đúng phiên đó — rút khỏi mẫu số của các ý tưởng trong phiên, các phiên khác
// vẫn tính. Đây là cách xử ca «tham dự không đầy đủ» mà không phải tắt cả
// người khỏi Hội đồng.
...
export const IdeaCouncilPhienPanel: React.FC<Props> = ({ round, items, phien }) => {
  const { taoPhien, suaPhien, xoaPhien, xepVaoPhien, moPhien, dongPhien, ghiVang, boVang } = useCouncilMutations(round.id);
  const { members } = useCouncilMembers(true);
  const { vang } = useDiemDanh(round.id, true);
  const [tenMoi, setTenMoi] = useState('');
  const [moDiemDanh, setMoDiemDanh] = useState<string | null>(null);
  const thanhVienHoatDong = members.filter(m => m.isActive);
          ...
          {cacPhien.map(p => {
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => setMoDiemDanh(moDiemDanh === p.id ? null : p.id)}
                  className={`flex items-center gap-1 px-2 py-1 rounded-lg font-bold text-2xs transition-all cursor-pointer ${
                    vang.some(v => v.phienId === p.id) ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'
                  }`}
                  title="Điểm danh phiên này — ai vắng thì rút khỏi mẫu số các ý tưởng trong phiên"
                >
                  <Users className="w-3 h-3" />
                  {vang.filter(v => v.phienId === p.id).length > 0
                    ? `${vang.filter(v => v.phienId === p.id).length} vắng`
                    : 'Điểm danh'}
                </button>
                <button
                  type="button"
                ...
                {moDiemDanh === p.id && (
                  <div className="w-full mt-1 p-2 rounded-lg bg-white border border-slate-200 space-y-1">
                    <p className="text-2xs text-slate-500">
                      Bấm tên để đánh dấu <b>vắng phiên này</b> (đến muộn / về sớm). Người vắng rút khỏi mẫu số
                      của {so} ý tưởng trong phiên; phiếu đã gửi vẫn tính. Vắng cả đợt thì đánh dấu ở khung Tiến độ chấm.
                    </p>
                    <div className="flex flex-wrap gap-1">
                      {thanhVienHoatDong.map(tv => {
                        const dong = vang.find(v => v.phienId === p.id && v.profileId === tv.profileId);
                        const vangCaDot = vang.some(v => !v.phienId && v.profileId === tv.profileId);
                        return (
                          <button
                            key={tv.profileId}
                            type="button"
                            disabled={vangCaDot}
                            onClick={() => {
                              if (dong) { void boVang(dong.id); return; }
                              void ghiVang(round.id, tv.profileId, p.id, `Vắng phiên «${p.ten}»`);
                            }}
                            title={vangCaDot ? 'Đã đánh dấu vắng cả đợt' : dong ? 'Bấm để bỏ đánh dấu vắng' : 'Bấm để đánh dấu vắng phiên này'}
                            className={`px-2 py-1 rounded-lg text-2xs font-bold transition-colors cursor-pointer disabled:cursor-not-allowed ${
                              vangCaDot ? 'bg-slate-100 text-slate-400 line-through'
                              : dong ? 'bg-rose-100 text-rose-700 border border-rose-300'
