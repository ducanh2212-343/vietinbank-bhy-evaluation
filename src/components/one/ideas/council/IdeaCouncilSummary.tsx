...
export const IdeaCouncilSummary: React.FC<IdeaCouncilSummaryProps> = ({ roundId, round, canPublish }) => {
  ...
  const nutCongBo = round && canPublish && (
    <button
      ...
      className={`ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold text-[11px] shadow-sm transition-all cursor-pointer ${round.resultsPublished ? 'bg-slate-600 hover:bg-slate-700 text-white' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}`}
    >
