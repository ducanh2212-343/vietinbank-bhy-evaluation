...
export async function downloadTheDiemExcel(th: TongHopTheDiem): Promise<void> {
  ...
  ws1.columns = [
    ...
    { header: 'Áp công thức', key: 'apCongThuc', width: 16 },
    ...IDEA_DEV_LEVELS.map(c => ({ header: `${c} (lũy kế)`, key: c, width: 14 })),
  ...
  for (const d of tinhTheDiem(th)) {
    ws1.addRow({
      ...
      apCongThuc: d.nhomApCongThuc === d.nhom ? '' : `${NHOM_VI_TRI_NGAN[d.nhomApCongThuc]} (tạm)`,
      ...Object.fromEntries(IDEA_DEV_LEVELS.map(c => [c, d.luyKe[c]])),
      quyDoi: d.diemQuyDoi,
  ...
  for (const r of th.phong.map(tinhPhongTheDiem)) {
    ws2.addRow({
      ...
      ...Object.fromEntries(IDEA_DEV_LEVELS.map(c => [c, r.luyKe[c]])),
      quyDoi: r.diemQuyDoi,
  ...
  [
    ...
    'Ghi nhận lũy kế: ý tưởng lên cấp cao vẫn được ghi nhận ở các cấp đã qua (10 ý tưởng, 2 Vươn cành, 1 Lan tỏa → 10 Ươm mầm · 2 Vươn cành · 1 Lan tỏa).',
    'Quy đổi Bén rễ: 1 Vươn cành = 2 Bén rễ, 1 Lan tỏa = 3 Bén rễ; Ươm mầm không quy đổi. Bằng Bén rễ + Vươn cành + Lan tỏa lũy kế.',
    ...
    'Phó phòng / Kiểm soát viên: TẠM tính như Trưởng phòng đơn vị mình cho tới khi có phân công cán bộ phụ trách.',
