...
- Báo cáo tổng hợp trả về qua hàm RPC `get_council_subject_report` (SECURITY DEFINER); danh tính người
  ...
  1. **Giám đốc Chi nhánh** (`council_members.member_group = 'giam_doc'`) và **tcth_admin/system_admin**:
     xem tất cả báo cáo, mọi kỳ, biên bản toàn kỳ, xuất Excel, gửi email kết quả.
  2. **Phó Giám đốc** (role `bgd` nhưng không phải Giám đốc): **chỉ xem đầu mối do mình phụ trách**
     ...
     Excel/gửi email trong phạm vi này. Ràng buộc cưỡng chế ngay trong RPC (không chỉ ẩn trên giao diện) —
     đã kiểm chứng bằng transaction rollback giả lập cả 2 vai trò.
