...
- **Khóa kết quả trong lúc chấm (embargo — bảo đảm khách quan):** mỗi kỳ có cờ
  `council_rounds.results_published` (mặc định `false`). Khi **chưa công bố**, CHỈ **Giám đốc Chi nhánh**
  (`member_group='giam_doc'`) và **Quản trị hệ thống** (`system_admin` — tài khoản `ducanh2212@gmail.com`)
  xem được báo cáo; **tất cả nhóm còn lại bị khóa** — gồm cả **TCTH admin** (Trưởng/Phó phòng TCTH, vì họ
  cũng là người chấm), PGĐ phụ trách và cán bộ đầu mối. Khi Hội đồng chấm xong, Giám đốc/Quản trị hệ thống
  bấm **“Mở kết quả cho mọi người”** trên trang *Báo cáo đầu mối* → `results_published=true` → áp dụng lại
  phân quyền xem thông thường (TCTH xem tất cả · PGĐ xem đầu mối phụ trách · cán bộ xem của mình). Có thể
  **Khóa lại** bất cứ lúc nào. Cưỡng chế ở tầng DB: RPC `get_council_subject_report` chặn theo embargo trước
  khi tới phân quyền; hành động công bố qua RPC `set_council_results_published` cũng chỉ Giám đốc + Quản trị
  hệ thống gọi được (TCTH bị từ chối). Trang *Phân tích đầu mối* cũng ẩn dữ liệu kỳ chưa công bố với TCTH.
  (Đã kiểm chứng trên DB bằng transaction rollback cho cả 4 vai trò, trước/sau công bố.)

- **Vai trò của Phó Giám đốc (quan trọng):** PGĐ là **user quản lý cấp Khối (role `pgd`)**, **KHÔNG phải admin**.
  Trước đây cả GĐ lẫn 3 PGĐ đều mang role `bgd` (admin), khiến PGĐ thấy toàn bộ menu/tab quản trị và mọi dữ
  liệu. Migration `20260707160000_demote_pgd_from_admin.sql` đã hạ 3 PGĐ (mọi hồ sơ active có chức danh
  "Phó giám đốc…") từ `bgd` → `pgd`; **Giám đốc Chi nhánh giữ nguyên `bgd`**. Hệ quả: `isAdmin=false` với PGĐ →
  tự động ẩn/chặn mọi mục menu và route admin (Quản trị Hội đồng đầu mối, Phân tích đầu mối, quản trị nhân sự,
  phân quyền, cài đặt…). PGĐ vẫn giữ: quyền quản lý cấp Khối (role `pgd`), quyền **chấm điểm Hội đồng** (theo
  `council_members`, không phụ thuộc vai trò), và quyền **xem báo cáo đầu mối mình phụ trách** (xem dưới).
  (Đã kiểm chứng trên DB: sau hạ quyền, PGĐ `has_role(bgd)=false`, không đọc được phiếu thô của người khác,
  nhưng RPC báo cáo đầu mối phụ trách vẫn trả dữ liệu.)
...
- Báo cáo tổng hợp trả về qua hàm RPC `get_council_subject_report` (SECURITY DEFINER); danh tính người
  chấm được ẩn danh bằng mã ổn định sinh từ id phiếu (không lộ qua API). Ba cấp quyền xem:
  1. **Giám đốc Chi nhánh** (`council_members.member_group = 'giam_doc'`, role `bgd`) và **tcth_admin/system_admin**:
     xem tất cả báo cáo, mọi kỳ, biên bản toàn kỳ, xuất Excel, gửi email kết quả.
  2. **Phó Giám đốc phụ trách** (role `pgd`): **chỉ xem đầu mối do mình phụ trách**
     (`council_subjects.supervisor_pgd_id` khớp hồ sơ của họ) + báo cáo của chính mình (khi họ là đầu mối
     cấp PGĐ được Hội đồng đánh giá) — không xem được báo cáo của 2 PGĐ còn lại hay đầu mối do PGĐ khác phụ
     trách. "Biên bản toàn kỳ" của họ tự động thu hẹp thành "Biên bản đầu mối tôi phụ trách", vẫn xuất được
     Excel/gửi email trong phạm vi này. Cưỡng chế ngay trong RPC theo `supervisor_pgd_id` (độc lập vai trò,
     vẫn đúng sau khi hạ quyền); giao diện nhận diện PGĐ phụ trách bằng dữ liệu (`useCouncilAccess.isSupervisor`),
     không dựa vào vai trò admin. Đã kiểm chứng bằng transaction rollback.
  3. **Cán bộ đầu mối khác / thành viên Hội đồng thường**: chỉ xem báo cáo của chính mình (nếu có), không
     có Excel/gửi email.
- **Trang *Phân tích đầu mối* (tổng hợp/so sánh toàn chi nhánh: xu hướng, bảng so sánh, radar):** chỉ dành cho
  **Giám đốc Chi nhánh + tcth_admin/system_admin**. **Phó Giám đốc KHÔNG truy cập** — đây là màn hình tổng hợp
  toàn chi nhánh, nếu để PGĐ xem sẽ lộ dữ liệu đầu mối do PGĐ khác phụ trách (không bảo đảm bảo mật giữa các
  PGĐ). PGĐ chỉ là user quản lý, xem báo cáo của đầu mối mình phụ trách tại *Báo cáo đầu mối*. Ẩn cả mục menu
  lẫn chặn khi vào thẳng URL; không nạp dữ liệu cho PGĐ.
...
## 6. Vận hành email nhắc/kết quả — lưu ý quan trọng

- Email hệ thống gửi từ **`noreply@343skill.com`** (máy chủ `notify.343skill.com`). Với Gmail,
  **lần đầu thư thường vào mục Spam hoặc tab Quảng cáo** — người nhận cần mở Spam, đánh dấu
  "Không phải spam" một lần là các thư sau vào Inbox.
- Email gửi tới **địa chỉ trong hồ sơ cán bộ** (cột email ở *Danh sách cán bộ*). Tooltip của nút
  nhắc ở tab *Tiến độ* và toast sau khi gửi đều hiển thị rõ địa chỉ nhận — nếu địa chỉ không đúng,
  sửa hồ sơ cán bộ trước rồi nhắc lại.
- Đã kiểm chứng pipeline 06/07/2026: email nhắc GĐ Trần Đức Anh trạng thái `sent` tới địa chỉ hồ sơ
  `alexteaman89@gmail.com` — Chi nhánh đối chiếu xem có đúng hộp thư GĐ đang dùng không.
- Tra soát lịch sử gửi: trang *Quản trị Email* (bảng `email_send_log`, template
  `council-vote-reminder` / `council-report`).
