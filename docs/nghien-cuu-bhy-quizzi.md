...
## 3b. Chiến dịch quiz toàn chi nhánh (bổ sung 07/2026)

Bên cạnh quiz tuần của phòng, **chiến dịch** là quiz lớn cả chi nhánh cùng làm:

| Khía cạnh | Thiết kế |
|---|---|
| Ai khởi tạo | Chỉ thành viên các phòng nghiệp vụ trụ sở: DVKH, KHDN, Bán lẻ, Hỗ trợ tín dụng, TCTH — danh mục `quiz_campaign_initiator_depts` (seed theo tên phòng, admin chỉnh được) |
| Phê duyệt | **Bắt buộc Ban Giám đốc** (role `bgd`): draft → pending → approved/rejected → closed. Guard trigger chặn mọi đường tự duyệt; từ chối phải ghi lý do; duyệt tự đóng dấu `approved_by/approved_at`. Cấu hình khoá sau khi gửi duyệt (rút về nháp mới sửa được) |
| Chống làm bài hộ | (1) **Mỗi người một đề** — `question_pool_size` (tùy chọn): bốc ngẫu nhiên N câu từ ngân hàng câu hỏi, thứ tự câu cũng ngẫu nhiên; (2) **Đảo thứ tự đáp án theo từng người** (`shuffle_options`): server lưu hoán vị riêng (`option_orders`), client chỉ thấy thứ tự hiển thị của mình, đáp án lưu về **chỉ số gốc** nên thống kê vẫn chuẩn; (3) toàn bộ cơ chế server-side của quiz phòng giữ nguyên (đáp án kín, đồng hồ server, 1 lượt/người) |
| Tổng hợp kết quả | Kiểu **Wooclap/Mentimeter** (`quiz_campaign_get_results`): thống kê từng câu — tỷ lệ đúng, phân bố lựa chọn từng phương án, số lượt hết giờ — **sắp theo câu sai nhiều nhất** để đưa vào đào tạo tiếp theo; kèm tỷ lệ tham gia theo phòng. Thống kê câu hỏi chỉ mở cho người ĐÃ hoàn thành hoặc người tổ chức (tránh lộ đề) |
| Ẩn danh | `anonymous_results` do người khởi tạo chọn: bật thì **không ai** (kể cả người khởi tạo) thấy tên người làm — RLS chặn SELECT attempts của người khác, danh sách người tham gia không được trả về; chỉ còn số liệu tổng hợp + mỗi người tự thấy kết quả mình |
| Xếp hạng | Không có leaderboard cá nhân toàn chi nhánh công khai (đúng nguyên tắc gamification). Người tổ chức thấy danh sách điểm khi KHÔNG ẩn danh; mọi người thấy tổng hợp phòng |
| Streak/badge | Hoàn thành chiến dịch **tính vào chuỗi tuần** cá nhân và các huy hiệu (Bền bỉ đếm gộp quiz phòng + chiến dịch qua `quiz_completed_total`; Thiện xạ/Tia chớp áp dụng nếu đề ≥5 câu) |

Bảng: `quiz_campaigns`, `quiz_campaign_questions` (đáp án kín), `quiz_campaign_attempts`
(đề riêng từng người: `question_ids[]` + `option_orders`), `quiz_campaign_answers` (chỉ số gốc).
RPC: `quiz_campaign_start_attempt`, `quiz_campaign_answer`, `quiz_campaign_get_review`,
`quiz_campaign_get_results`. UI: `/quizzi/chien-dich` (danh sách + hàng chờ BGĐ duyệt),
`/quizzi/chien-dich/tao-moi`, `/quizzi/chien-dich/:id` (engine làm bài dùng chung
