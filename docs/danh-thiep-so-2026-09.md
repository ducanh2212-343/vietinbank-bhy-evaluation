...
| Đường dẫn quản trị `/admin/danh-thiep/...`, tự phục vụ `/toi/danh-thiep` | `/quan-tri-vcard/{don-vi,chuc-danh,chuc-danh-rieng,can-bo}` và `/vcard` — màn «thẻ của tôi» đặt trong **khối tài khoản cá nhân** (ngăn «Thêm» trên điện thoại, menu tài khoản trên máy tính, bảng lệnh ⌘K) cạnh Hồ sơ cá nhân — tiện ích cá nhân nhỏ, không phải một cách vận hành nên không đứng trong Bắc Hưng Yên Ways; màn quản trị ở Quản trị chung → Tổ chức & Phân quyền (chốt 02/09/2026) | Theo quy ước đường dẫn tiếng Việt của cổng (`/quan-tri-khach`, `/quan-tri-ky-yeu`…). |
...
- Menu: «Danh thiếp VCard của tôi» trong khối tài khoản (`MobileNav.tsx`, `TopNav.tsx`, `CommandPalette.tsx`); `/vcard` gắn vào mục Hồ sơ cá nhân qua `extraPaths` để breadcrumb đúng; «Quản trị VCard» ở Quản trị chung (`src/lib/navigation.ts`).
