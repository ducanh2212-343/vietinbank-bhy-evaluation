...
| Level | Tên | Khung đề xuất | Cảm giác |
|---|---|---|---|
| L1 | Tân binh | Viền đồng, nền nhạt | Khởi đầu |
| L2 | Độc lập | Viền bạc, nền đậm hơn | Vững vàng |
| L3 | Chuyên gia | Viền vàng + tia sáng nhẹ | Uy tín |
| L4 | Bậc thầy | Viền kim cương/gradient thương hiệu VietinBank + glow | Hiếm, đáng khao khát |
| Chưa đạt | — | **Bóng đen (silhouette) trên nền khoá + ổ khoá nhỏ** | Tò mò, muốn mở |

**Khuyến nghị kỹ thuật giảm 90% công upload:** thay vì bắt admin upload 152 ảnh, dùng cơ chế
**compose 2 lớp**: `icon_url` của skill (đã có cột trong `skill_catalog`, upload 38 icon) +
khung level render bằng CSS/SVG. Ảnh trong `skill_level_images` trở thành *override tuỳ chọn*
cho skill nào muốn art riêng. Cách này bảo đảm đồng bộ thị giác tuyệt đối và có "ảnh" cho
mọi skill ngay ngày đầu. Nếu muốn art riêng từng skill × level, sinh bằng AI theo một
prompt-template thống nhất (cùng phong cách, cùng bố cục, chỉ đổi chủ thể + bậc khung) rồi
upload qua trang admin sẵn có.
